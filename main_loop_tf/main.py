...
def build_graph(placeholders, input_shape, build_model, which_set):
    ...
    # Process backwards, so that the summaries include all the summaries
    # of the lower devices
    for device, dev_inputs, dev_labels in zip(devices[::-1],
                                              inputs_per_gpu[::-1],
                                              labels_per_gpu[::-1]):
        device_str = device.replace('/', '').replace(':', '').lower()
        dev_set_str = '{}{}'.format(device_str, which_set)
    ...
    softmax_preds = softmax_pred[:n_splits]
    ...
    if is_training:
        ...
        for i in range(len(inputs_per_gpu)):
            update_ops += tf.get_collection(tf.GraphKeys.UPDATE_OPS,
                                            scope='gpu{}'.format(i))
        # Return a *list* of gradient update ops
